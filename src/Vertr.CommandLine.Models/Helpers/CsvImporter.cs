...
{
    public static IEnumerable<Candle> LoadCandles(string pathToCsv, string delimiter = ";")
    {
    ...
    private record class CandleDto
    {
        ...
        public Candle ToCandle()
            => new Candle
            {
                TimeUtc = DateTime.ParseExact($"{Date}T{Time}", "yyMMddTHHmmss", CultureInfo.InvariantCulture),
                Open = Open,
                High = High,
                Low = Low,
                Close = Close,
